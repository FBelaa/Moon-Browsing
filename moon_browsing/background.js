...
async function handleMessage(message) {
  switch (message.type) {
    ...
    case 'GET_FPS_STATUS': return { enabled: fpsTabs.has(message.tabId) };
    ...
    case 'OPEN_SPEEDTEST': await chrome.tabs.create({ url: 'https://www.speedtest.net/en' }); return { done: true };
...
async function toggleFps(tabId) {
  const enabled = !fpsTabs.has(tabId);
  ...
  if (enabled) {
    ...
    await chrome.declarativeNetRequest.updateSessionRules({ removeRuleIds: ruleIdsForTab(tabId) });
    await chrome.scripting.executeScript({ target: { tabId }, func: disablePageLiteMode });
  }
